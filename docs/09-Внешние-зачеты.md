            ...
            try {
                if ($action == 'add_external_credit') {
                    ...
                    if ($existing) {
                        ...
                        // ВАЖНО: В БД поле grade имеет NOT NULL, поэтому передаем пустую строку вместо null
                        ...
                        if (empty($record->studentid) || empty($record->subjectid) || empty($record->createdby)) {
                            ...
                            try {
                                ...
                                // Обработка ошибок БД с детальными сообщениями
                                // (см. раздел 10.3)
                            }
                        }
...
## Шаг 10: Важные моменты при работе с данными

### 10.1. Обработка NOT NULL полей

**ВАЖНО:** В реальной базе данных многие поля могут иметь `NOT NULL`, даже если в схеме они определены как `NOTNULL="false"`. Это может произойти при миграции или изменении структуры БД.

Поэтому при вставке данных нужно передавать значения по умолчанию вместо `NULL`:
...
### 10.2. Валидация данных перед вставкой

Перед вставкой данных обязательно проверяем обязательные поля:
...
### 10.3. Детальная обработка ошибок

Для администраторов важно показывать детальную информацию об ошибках:
...
### Как добавить внешний зачет через интерфейс:

1. **Войдите как администратор** в Moodle
2. **Перейдите на страницу студента** с параметром `studentid`:
   ```
   /local/deanpromoodle/pages/student.php?studentid=123&tab=programs&subtab=external_credits
   ```
   Или найдите студента через поиск на странице администратора/преподавателя.

3. **Нажмите кнопку "Добавить внешний зачет"** (видна только администраторам)

4. **Заполните форму**:
   - **Предмет** (обязательно): Выберите предмет из выпадающего списка
   - **Оценка**: Введите текстовую оценку (например, "3", "4", "5" или процент)
   - **Оценка в процентах**: Введите число от 0 до 100 (например, 95)
   - **Учебное заведение (из списка)**: Опционально выберите из списка, если оно есть в системе
   - **Название учебного заведения** (обязательно): Введите название учебного заведения вручную
   - **Дата зачета**: Выберите дату в календаре (формат: ГГГГ-ММ-ДД)
   - **Номер документа**: Введите номер документа о зачете (опционально)
   - **Примечания**: Добавьте дополнительные примечания (опционально)

5. **Нажмите "Добавить"**

6. **Проверьте результат**: Внешний зачет должен появиться в таблице внешних зачетов
...
- **Название учебного заведения**: "Московская богословская семинария"
...
## Шаг 11: Индикация внешних зачетов в таблице программы

При просмотре программы студента (`student.php?action=viewprogram&programid=X`) для предметов с внешними зачетами отображается специальный индикатор:
...
- В колонке "Статус завершения" - синий значок с текстом "Внешний зачет"
- В колонке "Оценка" - значок университета
- При наведении показывается название учебного заведения
...
9. ✅ Обработка ошибок с детальными сообщениями для администраторов
10. ✅ Правильная обработка NOT NULL полей (передача пустых строк/0 вместо NULL)
11. ✅ Валидация данных перед вставкой
12. ✅ Индикация внешних зачетов в таблице программы
...
### Как добавить внешний зачет?

1. Войдите как администратор
2. Перейдите на страницу студента: `/local/deanpromoodle/pages/student.php?studentid=XXX&tab=programs&subtab=external_credits`
3. Нажмите "Добавить внешний зачет"
4. Заполните обязательные поля: Предмет и Название учебного заведения
5. Нажмите "Добавить"

### Почему возникает ошибка "Ошибка записи в базу данных"?

Эта ошибка может возникать по нескольким причинам:
- Не заполнены обязательные поля (студент, предмет, название учебного заведения)
- Предмет не существует в базе данных
- Учебное заведение с указанным ID не найдено
- Внешний зачет по этому предмету уже существует для данного студента
- Проблемы с NOT NULL полями (нужно передавать пустые строки/0 вместо NULL)

Для администраторов отображаются детальные сообщения об ошибках с указанием конкретной причины.

### Можно ли редактировать внешний зачет?

Да, нажмите кнопку редактирования (иконка карандаша) рядом с записью внешнего зачета в таблице.

### Можно ли удалить внешний зачет?

Да, нажмите кнопку удаления (иконка корзины) рядом с записью. Подтвердите удаление в диалоговом окне.
