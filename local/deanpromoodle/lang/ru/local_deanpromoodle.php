...
$string['feed_help'] = 'Показаны регистрации со ролью «Студент» за последние 90 дней с портала МБС (настраивается в параметрах плагина local_deanpromoodle). Колонка «Форма»: зелёная галочка — заполнены обязательные поля в «Дополнительные данные» (ФИО, дата рождения, телефон, email, паспортные данные); предупреждение — чего-то не хватает. Скрытие не удаляет пользователя — запись только убирается из списка; во вкладке «Скрытые» можно вернуть.';
...
$string['applicants_filter_mode_desc'] = '«Только портал МБС» — в списке только пользователи, для которых сработал фильтр (сайт в поле веб-страницы, доп. поле профиля, домен email или способ аутентификации — см. ниже). «Все регистрации» — все новые студенты за период, как раньше.';
...
$string['applicants_source_hosts_desc'] = 'Через запятую. Пользователь считается с портала МБС, если в поле «Веб-страница» в профиле Moodle встречается эта подстрока (по умолчанию mbs.russianseminary.org).';
