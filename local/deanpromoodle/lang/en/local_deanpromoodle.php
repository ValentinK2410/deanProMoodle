...
$string['feed_help'] = 'Shows Student role registrations from the last 90 days from the MBS portal (configure in local_deanpromoodle settings). Column «Form»: green check — required fields in Additional data are filled; warning — something is missing. Hiding only removes the row from this list; restore from the Hidden tab.';
...
$string['applicants_filter_mode_desc'] = '«MBS portal only» lists users who match the rules below (website field, profile field, email domain, or auth plugin). «All registrations» lists every new student in the period (legacy behaviour).';
...
$string['applicants_source_hosts_desc'] = 'Comma-separated. A user matches if their Moodle profile «Web page» field contains this substring (default: mbs.russianseminary.org).';
